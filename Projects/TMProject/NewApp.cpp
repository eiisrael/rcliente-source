...
HRESULT NewApp::Initialize(HINSTANCE hInstance, int nFull)
{
	...
	unsigned int targetH = stResList[(nResIndex > 0 ? nResIndex - 1 : 1)].dwHeight;
	unsigned int targetBpp = stResList[(nResIndex > 0 ? nResIndex - 1 : 1)].dwBit;

	// Resolução base de referência para UI/layout (manter proporção)
	const float baseW = 800.0f;
	const float baseH = 600.0f;

	// Calcula fator de escala UI para preservar proporção (não distorce)
	float scaleW = (float)targetW / baseW;
	float scaleH = (float)targetH / baseH;
	float uiScale = (scaleW < scaleH) ? scaleW : scaleH;
	if (uiScale <= 0.0f)
		uiScale = 1.0f;

	// Ajusta a resolução de render para manter proporção baseada na resolução base
	int adjustedW = (int)(baseW * uiScale + 0.5f);
	int adjustedH = (int)(baseH * uiScale + 0.5f);

	// Evita 0
	if (adjustedW <= 0) adjustedW = 800;
	if (adjustedH <= 0) adjustedH = 600;

	m_dwScreenWidth = adjustedW;
	m_dwScreenHeight = adjustedH;
	m_dwColorBit = targetBpp;

	// Calcula e aplica tamanho de fonte proporcional à escala UI, com limites
	int referenceFont = 13;
	// usa largura alvo original para mapear um _nFontSize de referência (preserva intenção original)
	switch (targetW)
	{
	case 640:
		referenceFont = 12;
...
HRESULT NewApp::MsgProc(HWND hWnd, DWORD uMsg, DWORD wParam, int lParam)
{
	...
	case WM_INPUTLANGCHANGE:
	{
		...
		else if (m_pEventTranslator != nullptr)
		{
				...
				g_pCurrentScene->m_pControlContainer->m_pFocusControl->m_eCtrlType == CONTROL_TYPE::CTRL_TYPE_EDITABLETEXT)
			{
				...
				if (!strcmp(static_cast<TMFieldScene*>(g_pCurrentScene)->m_pTextIMEDesc->GetText(), "�"))
	...
	case WM_ACTIVATE:
	{
		if (g_pCurrentScene != nullptr)
		{
			if (strcmp(g_pCurrentScene->m_pTextIMEDesc->GetText(), "�"))
