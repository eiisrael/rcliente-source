...
// Seguro: não dereferencia "ctrl" em nenhum momento.
bool IsControlAlive(SControlContainer* container, const SControl* ctrl)
{
	if (!container || !ctrl)
		return false;

	SControl* pCurrent = container->m_pControlRoot;
	SControl* pRoot = container->m_pControlRoot;
	...
	do
	{
		if (!pCurrent->m_cDeleted)
		{
			// Comparação por endereço: sempre segura, mesmo se ctrl estiver dangling.
			if (pCurrent == ctrl)
				return true;

			if (pCurrent->m_pDown)
			{
				pCurrent = static_cast<SControl*>(pCurrent->m_pDown);
				continue;
			}
		}
		...
		do
		{
			if (pCurrent->m_pNextLink != nullptr)
			{
				pCurrent = static_cast<SControl*>(pCurrent->m_pNextLink);
				break;
			}

			pCurrent = static_cast<SControl*>(pCurrent->m_pTop);
...
void ObjectManager::RenderControl()
{
	...
	if (pBaseScene && pBaseScene->m_eSceneType == ESCENE_TYPE::ESCENE_FIELD)
	{
		...
		if (pScene->m_pMiniPanel && pScene->m_pMiniPanel->m_cDeleted)
		{
			...
			pScene->m_pMiniPanel = nullptr;
		}

		if (pScene->m_pMiniPanel && pScene->m_bMiniPanelAnimating)
		{
		...
		// ================= INVENTÁRIO =================
		if (pScene->m_pInvenPanel)
		{
			if (!IsControlAlive(pScene->m_pControlContainer, pScene->m_pInvenPanel))
			{
				pScene->m_bInvenAnimating = 0;
				pScene->m_bInvenClosing = 0;
				pScene->m_pInvenPanel = nullptr;
			}
