...
int SPanel::OnMouseEvent(unsigned int dwFlags, unsigned int wParam, int nX, int nY)
{
	...
	case WM_LBUTTONUP:
	{
		...
		// LOG (anti-spam) - só se soltou clique em cima do panel
		if (m_bOver == 1)
		{
			static DWORD s_lastTime = 0;
			static DWORD s_lastId = 0;

			DWORD now = GetTickCount();
			DWORD id = (DWORD)m_dwControlID;

			// evita spam repetindo o mesmo id muito rápido
			if (!(id == s_lastId && (now - s_lastTime) < 200))
			{
				s_lastId = id;
				s_lastTime = now;

				printf("[PANEL UP] id=%u ptr=%p pos=(%.0f,%.0f) size=(%.0f,%.0f) pickable=%d picked=%d modal=%d\n",
					(unsigned)id,
					this,
					m_nPosX, m_nPosY,
					m_nWidth, m_nHeight,
...
{
	if (m_GCPanel.nTextureSetIndex >= 0 || (m_GCPanel.dwColor & Black))
	{
...
S3DObj::S3DObj(int nObjIndex, float inX, float inY, float inWidth, float inHeight)
	: SControl(inX, inY, inWidth, inHeight)
{
	m_GCObj = GeomControl(RENDERCTRLTYPE::RENDER_3DOBJ, 0, 0.0f, 0.0f, inWidth, inHeight, 0, White);
...
void SText::SetText(char* istrText, int bCheckZero)
{
	...
	if (m_cComma == 2)
	{
		...
		m_Font4.SetText(m_GCText4.strString, White, bCheckZero);
	}
}
...
void SText::SetTextColor(unsigned int dwFontColor)
{
	if (RenderDevice::m_nBright > 58)
	{
		...
		dwFontColor = nB | (nG << 8) | dwFontColor & Black | (nR << 16);
	}
...
void SEditableText::Update()
{
	...
	m_GCText.pFont->SetText(m_GCText.strString, White, 0);
}
	...
	if (istrText != nullptr)
	{
		if (inTextureSetIndex >= -2 && inTextureSetIndex < 0)
		{
			...
			if (m_GCPanel.pFont != nullptr)
				m_GCPanel.pFont->SetText(m_GCPanel.strString, White, 0);
		}

		m_pAltText = new SText(-1, istrText, White, inWidth - 10.0f, 0.0f, (float)strlen(istrText) * 8.0f, (float)RenderDevice::m_nFontSize,
			0, 0xFF333333, 1u, 0);
...
int SButton::OnMouseEvent(unsigned int dwFlags, unsigned int wParam, int nX, int nY)
{
	...
	if (m_pAltText != nullptr)
	{
		...
		else
		{
			...
			if (m_GCPanel.pFont != nullptr)
				m_GCPanel.pFont->m_dwShadeColor = Black;
		}
	}
	...
	: SPanel(inTextureSetIndex, inX, inY, inWidth, inHeight, idwColor, RENDERCTRLTYPE::RENDER_IMAGE_STRETCH)
{
	...
	if (RenderDevice::m_nBright > 58)
	{
		...
		m_GCProgress.dwColor = nB | (nG << 8) | idwProgressColor & Black | (nR << 16);
	}
	...
	: SControl(inX, inY, inWidth, inHeight)
{
	...
	if (m_dwStyle == 0)
	{
		...
		if (bChat)
			nStartXPos = -5;
		m_pBackground1 = new SPanel(510, (float)nStartXPos, 0.0f, 13.0f, inHeight, White, RENDERCTRLTYPE::RENDER_IMAGE_STRETCH);
		m_pBackground1->GetGeomControl()->eRenderType = RENDERCTRLTYPE::RENDER_IMAGE_STRETCH;
		m_pBar = new SPanel(176, (float)nStartXPos - 0.5f, 10.0f, 14.5f - 2.0f, 11.0f, White, RENDERCTRLTYPE::RENDER_IMAGE_STRETCH);
...
void SScrollBar::Update()
{
	...
	if (m_nMax > 0)
	{
		...
		if (m_dwStyle == 0)
			...
			m_pBar->SetPos((float)m_nScrollPos + m_nBtnSize, 10.0f);
		...
		dwAlignType)
{
	...
	m_pBackSelection = new SPanel(566, inX - 2.0f, inY, inWidth, inHeight, White, RENDERCTRLTYPE::RENDER_IMAGE_STRETCH);
...
void SListBoxItem::FrameMove2(stGeomList* pDrawList, TMVector2 ivItemPos, int inParentLayer, int nFlag)
{
	...
	{
		m_pBackSelection->m_nWidth = m_pMainListBox->m_nWidth;
		m_pBackSelection->m_nHeight = m_nHeight + 3.7f;
...
SListBoxBoardItem::SListBoxBoardItem(char* szIndex, char* szVIndex, char* szTitle, char* szWriter, char* szCount, char* szDate, unsigned int dwColor, int bTitile)
	: SListBoxItem(szVIndex, White, 0.0f, 0.0f, 500.0f, 12.0f, 0, 0x77777777, 1, 0)
{
	unsigned int dwFontColor = White;
		...
		0)
{
	...
	m_pHpProgress = new SProgressBar(7, nHp, nMaxHp, 0.0f, 15.0f, 110.0f, 8.0f, White, White, 1);
...
void SListBoxPartyItem::FrameMove2(stGeomList* pDrawList, TMVector2 ivItemPos, int inParentLayer, int nFlag)
{
	...
	if (m_nState == 1)
	{
		if ((g_pTimerManager->GetServerTime() % 1000) / 500)
			m_GCText.dwColor = Red;
		else 
			m_GCText.dwColor = White;
		...
		0)
{
	...
	if (m_cCastle == 1)
		m_pCrownPanel = new SPanel(151, 24.0f + 104.0f, 1.0f, 16.0f, 16.0f, White, RENDERCTRLTYPE::RENDER_IMAGE_STRETCH);
	if (m_cGoldBug == 1)
		m_pGoldBugPanel = new SPanel(316, 24.0f + 122.0f, 1.0f, 16.0f, 16.0f, White, RENDERCTRLTYPE::RENDER_IMAGE_STRETCH);
...
void SListBox::SetEditable()
{
	if (m_pEditLine == nullptr)
	{
		...
		m_pEditLine = new SEditableText(-2,	"", 79,	0, White, 0.0f, 0.0f, m_nWidth, 16.0f, 0, 0, 1, 0);
		if (m_pEditLine != nullptr)
		{
...
int SListBox::OnControlEvent(DWORD idwControlID, DWORD idwEvent)
{
	...
	if (idwEvent == 0|| idwEvent == 8)
	{
		...
		SListBoxItem* pItem = new SListBoxItem(szText, White, 0.0f, 0.0f, 300.0f, 16.0f, 0, 0x77777777, 1, 0);
		AddItem(pItem);
	...
	: SPanel(-2, inX, inY, 256.0f, 172.0f, 0x1010101, RENDERCTRLTYPE::RENDER_IMAGE_STRETCH)
{
	...
	m_pMessage = new SText(-1, istrMessage, White, 20.0f, 36.0f, (float)strlen(istrMessage) * 6.0f, 52.0f, -1, 1, 0, 0);
	m_pMessage2 = new SText(-1, istrMessage, White, 20.0f, 52.0f, (float)strlen(istrMessage) * 6.0f, 52.0f, -1, 1, 0, 0);
	m_pCaption = new SText(-1, g_pMessageStringTable[237], White, 96.0f, 9.0f,
	...
	: SPanel(-45, inX, inY, inWidth, inHeight, 0xAAFFFFFF, RENDERCTRLTYPE::RENDER_IMAGE_STRETCH)//new
{
	...
	m_pText = new SText(-2, istrMessage, White, 20.0f, 1.0f, (float)(inWidth - 2.0f) - 50.0f, inHeight - 2.0f, 1, 0, 1, 1);
	m_pText2 = new SText(-2, "", White, 50.0f, 24.0f, (float)(inWidth - 2.0) - 50.0f, 14.0f, 1, 0, 1, 1);
