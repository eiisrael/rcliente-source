...
TMEffectSkinMesh::~TMEffectSkinMesh()
{
	...
	{
		TMSkillFire* pFire = new TMSkillFire(m_vecTargetPos, 0, nullptr, White, 0x22331100);
...
int TMEffectSkinMesh::InitObject(int bExpand)
{
	if (m_nSkinMeshType2 >= 0)
	{
		...
		if (m_nMotionType == 10 && m_pSkinMesh)
		{
			...
			m_pSkinMesh->m_dwStartOffset = 1000;
		}
...
int TMEffectSkinMesh::FrameMove(unsigned int dwServerTime)
{
	...
	const static unsigned int dwCol[3]{ 0xFFFFAA00, Red, 0xFFFFAAFF };
	...
	if (fProgress >= 0.0099999998f)
	{
		...
		case 3:
		{
			...
			auto pChild = new TMSkillPoison({ m_vecPosition.x, m_fHeight, m_vecPosition.y }, Red, 2, 1, 0);
		...
		case 10://asa da ht
		{
			if (m_pOwner)
			{
				...
				m_vecPosition.x = (m_pOwner->m_vecTempPos[2].x + m_pOwner->m_vecTempPos[2].x) / 2.0f;
				m_vecPosition.y = (m_pOwner->m_vecTempPos[2].z + m_pOwner->m_vecTempPos[2].z) / 2.0f;
		...
		case 11://asa do traje
		{
			...
			if (m_pSkinMesh)
			{
				m_pSkinMesh->m_dwFPS = 21* m_dwLifeTime / 1000u;
				m_pSkinMesh->SetAnimation(g_MobAniTable[m_nSkinMeshType].dwAniTable[2]);
			}
		}
		break;
