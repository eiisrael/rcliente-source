...
// Chat resize animation
bool  m_bChatResizeAnim = false;
DWORD m_dwChatResizeStart = 0;
float m_fChatResizeFromH = 0.0f;
float m_fChatResizeToH = 0.0f;
	...
	: TMScene()
{
	...
	m_pMiniPanel = nullptr;
	m_bMiniPanelAnimating = 0;
	m_bMiniPanelClosing = 0;
	//
	m_fInvenAnimX = 0.0f;
	m_fInvenTargetX = 0.0f;
	m_fInvenSpeedOpen = 35.0f;
	m_fInvenSpeedClose = 45.0f;
	m_bInvenAnimating = 0;
	m_bInvenClosing = 0;
...
void SetMinimapPos()
{
	...
	if (fpMinimap)
	{
		...
		for (int index = 0; index < 256; ++index)
		{
			...
			{
			case 1:
				g_MinimapPos[index].dwColor = White;
				break;
			case 2:
...
void TMFieldScene::UpdateChatResizeAnim()
{
	if (!m_bChatResizeAnim)
		return;

	if (!m_pChatList || !m_pChatBack || !m_pControlContainer)
	{
	...
	// reposiciona em Y baseado no painel
	...
	if (pChatPanel)
	{
		...
		m_pChatBack->SetPos(m_pChatList->m_nPosX, chatBaseY);
	}
	...
	if (t >= 1.0f)
	{
		...
		if (pChatPanel2)
		{
			float chatBaseY = UI_SnapPx((float)(pChatPanel2->m_nPosY - finalH) - 4.0f);
			float chatTextUp = UI_SnapPx(BASE_ScreenResize(3.0f));
...
int TMFieldScene::InitializeScene()
{
	...
	m_pQuizQuestion = new SText(-2, "Question", White, 5.0f, 104.0f, 510.0f, 16.0f, 0, 0x77777777, 1, 0);
	if (m_pQuizQuestion)
...
#pragma region ChatPanel(Painel do Chat)
	...
	if (pChatPanel)
	{
		pChatPanel->m_nPosX += offsetX;
		pChatPanel->m_nPosY += offsetY;
	}

	m_pChatList->m_nPosX += offsetX;
	m_pChatBack->m_nPosX += offsetX;
	...
	{
		m_pChatSelectPanel->m_nPosX += offsetX;
		m_pChatSelectPanel->m_nPosY += offsetY;
	}

	if (m_pChatListPanel)
	{
		m_pChatListPanel->m_nPosX += offsetX;
		m_pChatListPanel->m_nPosY += offsetY;
	}

	if (m_pChatType)
	{
		m_pChatType->m_nPosX += offsetX;
		m_pChatType->m_nPosY += offsetY;
	}
	...
	if (m_pMainInfo2)
	{
		...
		m_pMainInfo2->m_nPosX += offsetX;
		m_pMainInfo2->m_nPosY += offsetY;
	}
	...
	if (m_pChatList)
		printf("[CHATLIST] id=%u ptr=%p pos=(%.0f,%.0f) size=(%.0f,%.0f)\n",
			(unsigned)m_pChatList->m_dwControlID, m_pChatList,
			m_pChatList->m_nPosX, m_pChatList->m_nPosY,
			m_pChatList->m_nWidth, m_pChatList->m_nHeight);

#pragma endregion


#pragma region MiniMapPanel(Painel do Mini Mapa)
...
#pragma region Cargo Painel (Bau)
...
#pragma region Inventory Panel(Painel de Inventario)
	...
	if (m_pInvenPanel)
	{
		float finalPosX = RenderDevice::m_fWidthRatio * 650.0f;
		float finalPosY = RenderDevice::m_fHeightRatio * 35.0f;

		// guarda o destino final
		m_fInvenTargetX = finalPosX;

		// começa fora da tela (lado direito)
		float screenRight = (float)g_pDevice->m_dwScreenWidth;
		m_fInvenAnimX = screenRight + 20.0f;

		m_pInvenPanel->SetPos(m_fInvenAnimX, finalPosY);
...
#pragma region Auto Trade Panel(Painel de Auto Trade)
		//PAINEL DO AUTO TRADE AUTOTRADE
	...
	if (m_pCPanel)
	{
		// alvo: onde ele deve ficar quando ABERTO (use o X original dele)
		m_fCPanelTargetX = m_pCPanel->m_nPosX;

		// começa fora da tela (à direita)
		float screenRight = (float)g_pDevice->m_dwScreenWidth;
		m_fCPanelAnimX = screenRight + BASE_ScreenResize(40.0f);
		m_pCPanel->m_nPosX = m_fCPanelAnimX;

		// velocidades simples
		m_fCPanelSpeedOpen = BASE_ScreenResize(25.0f);
		m_fCPanelSpeedClose = BASE_ScreenResize(20.0f);

		m_pCPanel->SetVisible(0);

		m_bCPanelAnimating = 0;
		m_bCPanelClosing = 0;
	}
	...
	m_pSkillMPanel->SetPos(RenderDevice::m_fWidthRatio * 130.0f,
		RenderDevice::m_fHeightRatio * 35.0f);
	...
	m_pStorePageBtn4 = (SButton*)m_pControlContainer->FindControl(67335);
	...
	m_pCargoGridList[3] = (SGridControl*)m_pControlContainer->FindControl(67331);
	...
	m_pCargoGridList[3]->SetVisible(0);
	...
	m_pCargoGridList[3]->m_bDrawGrid = 1;
	...
	m_plojapessoal = (SPanel*)m_pControlContainer->FindControl(647);
	...
	{
		///////////////////////Loja pessoal///////////////////////////
		m_plojapessoal->m_nWidth = BASE_ScreenResize(205.0f);
		...
		m_pCargoGridList[3]->m_nPosX = BASE_ScreenResize(13.0f);
		...
		m_pCargoGridList[3]->m_nWidth = BASE_ScreenResize(214.0f);
		m_pStorePageBtn1->m_nPosX = BASE_ScreenResize(140.0f);
		m_pStorePageBtn2->m_nPosX = BASE_ScreenResize(160.0f);
		m_pStorePageBtn3->m_nPosX = BASE_ScreenResize(180.0f);
		m_pStorePageBtn4->m_nPosX = BASE_ScreenResize(200.0f);
	...
	m_pQuick_Sloat[0]->m_nPosX = BASE_ScreenResize(231.5f);// alterado
	...
	m_pQuick_Sloat[3]->m_nPosX = BASE_ScreenResize(309.0f);// alterado 
  ...
  if(g_pItemList[pMobData->Equip[4].sIndex].nPos == 16)
	...
	m_pTarget2 = new TMEffectMesh(317, Red, 0.0f, 0);
	...
	m_pAlphaNative->SetPos((float)(m_pEditChat->m_nWidth + m_pChatPanel->m_nWidth) - 46.0f,		 // BR DO CHAT
...
int TMFieldScene::OnControlEvent(unsigned int idwControlID, unsigned int idwEvent)
{
				...
				if (m_nLastAutoTradePos >= 0)
				{
					for (int j = 0; j < 10; ++j)
					{
						...
						if (!pAutoTradeItem)
						{
							...
							pCargoItem->m_GCObj.dwColor = Red;
						...
						if (pAutoTradeItem && j == 11)
						{
							...
							pCargoItem->m_GCObj.dwColor = White;
						}
					}
					m_nLastAutoTradePos = -1;
				}
	...
	if (idwControlID == B_CHAR_CLOSE)
	{
		...
		// garante ponteiro válido
		SPanel* pPanel = (SPanel*)m_pControlContainer->FindControl(65696);
		if (!pPanel)
			return 0;

		m_pCPanel = pPanel;

		// se já está animando, ignora (evita bug)
		if (m_bCPanelAnimating)
			return 0;

		// se já está invisível, só ajusta botão/foco e sai
		if (!m_pCPanel->m_bVisible)
		{
			SButton* pCharBtn = (SButton*)m_pControlContainer->FindControl(B_CHAR);
			if (pCharBtn)
				pCharBtn->SetSelected(0);
		...
		// ===== FECHAR ANIMADO (pra esquerda) =====
		const float offPad = BASE_ScreenResize(10.0f);
		const float closeX = -m_pCPanel->m_nWidth - offPad;

		m_bCPanelClosing = 1;
		m_bCPanelAnimating = 1;

		m_fCPanelAnimX = m_pCPanel->m_nPosX; // começa do X atual
		m_fCPanelTargetX = closeX;             // destino: fora da tela à esquerda

		// botão char fica "desmarcado"
		SButton* pCharBtn = (SButton*)m_pControlContainer->FindControl(B_CHAR);
		if (pCharBtn)
			pCharBtn->SetSelected(0);
	...
	if (idwControlID == B_CHAR)
	{
		...
		// AGORA abre/fecha com animação (igual tecla C e igual inven animado)
		ToggleCharInfoAnimated();
		return 0;
	}
	...
	if (idwControlID == B_CHATLIST_LIGHT)	  /// BRILHO DO CHAT
	{
		// Sempre começa no mais claro quando o jogo abrir (ou quando essa função rodar pela 1ª vez)
		static int s_step = 0;

		// NUNCA fica invisível (sem 0x00000000)
		static const DWORD kColors[] =
		{
			0x66000000, // mais claro
			0x88000000,
			0xAA000000,
			0xFF000000  // preto total (Black)
		};

		if (m_pChatBack)
		{
			m_pChatBack->m_GCPanel.dwColor = kColors[s_step];
			m_pChatBack->Update(); // garante refresh imediato
		}

		s_step = (s_step + 1) % (int)(sizeof(kColors) / sizeof(kColors[0]));
		return 0;
	}
	...
	if (idwControlID == B_CARGO_PAGE1)
	{
		...
		m_pStorePageBtn4->SetTextureSetIndex(177);//botao guarda
		return 0;
	}
	if (idwControlID == B_CARGO_PAGE2)
	{
		...
		m_pStorePageBtn3->SetTextureSetIndex(535);
		m_pStorePageBtn4->SetTextureSetIndex(177);
		return 0;
	}
	...
	if (idwControlID == B_CARGO_PAGE4)
	{
		m_pCargoGridList[0]->SetVisible(0);
		m_pCargoGridList[1]->SetVisible(0);
		m_pCargoGridList[2]->SetVisible(0);
		m_pCargoGridList[3]->SetVisible(0);
		m_pCargoGrid = m_pCargoGridList[3];
		m_pCargoGrid->SetVisible(1);
		m_pStorePageBtn1->SetTextureSetIndex(531);
		m_pStorePageBtn2->SetTextureSetIndex(533);
		m_pStorePageBtn3->SetTextureSetIndex(535);
		m_pStorePageBtn4->SetTextureSetIndex(178);
		return 0;
	}
	...
	if (idwControlID == P_MINIBTNPANEL_BTN)			 // MINI PAINEL
	{
		if (!m_pMiniPanel)
			return 0;

		bool willOpen = !m_pMiniPanel->m_bVisible;
		...
		if (m_pMiniBtn)
			m_pMiniBtn->m_bSelected = willOpen;
		...
		if (pServerList)
		{
			...
			for (int num = 1;; ++num)
			{
				...
				if (g_pServerList[currentServerGroupIndex][num][0])
				{
					...
					// -1??
					auto server = new SListBoxServerItem(nTextureSet, iStrText, White, 0.0f, 0.0f, static_cast<float>(g_nChannelWidth), 16.0f, nCount, 0, 0, num);
				...
				else if (serverGroup == currentServerGroupIndex && num < serverGroup)
				{
					...
					auto server = new SListBoxServerItem(6, iStrTexr, White, 0.0f, 0.0f, static_cast<float>(g_nChannelWidth), 16.0f, nUserCount2[num], 0, 0, 0);
					if (nUserCount[num] < 0)
						server->m_cConnected = 0;
		...
		nY >= static_cast<int>(g_pDevice->m_dwScreenHeight - g_pDevice->m_nHeightShift))
	{
		...
		if (g_pObjectManager->m_stMobData.CurrentScore.Hp > 0)
		{
			...
			if (nX > 0 && nY > 0 && nX < SWidth && nY < SHeight)
			{
				...
				{
					bool clickInsideChat = false;

					if (m_pChatBack && m_pChatBack->IsVisible())
					{
						TMVector2 pos = m_pChatBack->GetPos();

						if (nX >= pos.x &&
							nX <= pos.x + m_pChatBack->m_nWidth &&
							nY >= pos.y &&
							nY <= pos.y + m_pChatBack->m_nHeight)
						{
							clickInsideChat = true;
						}
					}
...
int TMFieldScene::OnPacketEvent(unsigned int dwCode, char* buf)
{
	...
	case 0x2132:
		return OnPacketInforPlay(reinterpret_cast<stSendInfoClient*>(pStd));
		break;
...
{
	UpdateChatResizeAnim();



	// garante que cargo nunca exista sem inventario
	if (m_pCargoPanel && m_pCargoPanel->IsVisible())
	{
	...
	for (int i =0; i < 32; ++i)
	{
		if ((unsigned int)m_pMyHuman->m_stAffect[i].Type > 0)
		{
			...
			if (sTime <= 0)
			{
				...
				{
					if (!i)
						m_pAffect[0]->SetTextColor(Black);
			...
			if (m_pAffectL[i])
			{
				if (!i)
				{
					if (sTime < 7375)
					{
						...
						m_pAffect[i]->SetTextColor(White);
				}
	...
	if (m_pRankTimeText && m_bRankTimeOn)
	{
		if (m_pGround->m_vecOffsetIndex.x == 1 && m_pGround->m_vecOffsetIndex.y == 31
			...
			|| m_pAutoTrade->m_bVisible && m_pGround->m_vecOffsetIndex.x == 16 && m_pGround->m_vecOffsetIndex.y == 16//armia //greentime
			|| m_pAutoTrade->m_bVisible && m_pGround->m_vecOffsetIndex.x == 8 && m_pGround->m_vecOffsetIndex.y == 13//noatum //greentime
			|| m_pAutoTrade->m_bVisible && m_pGround->m_vecOffsetIndex.x == 28 && m_pGround->m_vecOffsetIndex.y == 24//gelo //greentime
			|| m_pAutoTrade->m_bVisible && m_pGround->m_vecOffsetIndex.x == 19 && m_pGround->m_vecOffsetIndex.y == 13//azran //greentime
			|| m_pAutoTrade->m_bVisible && m_pGround->m_vecOffsetIndex.x == 19 && m_pGround->m_vecOffsetIndex.y == 15)//erion //greentime
		{
	...
	for (int i = 0; i < m_pPartyList->m_nNumItem; ++i)
	{
		...
		if (pHuman)
		{
			...
			{
				pPartyItem->m_GCText.dwColor = Red;
			...
			{
				pPartyItem->m_GCText.dwColor = White;
				pPartyItem->m_GCText.pFont->SetText(pPartyItem->m_GCText.strString, pPartyItem->m_GCText.dwColor, 0);
			}
		}
	}
...
void TMFieldScene::CloseCargoSafe()	  // fechamento do cargo arrumado!
{
	if (!m_pCargoPanel || !m_pCargoPanel->IsVisible())
		return;
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			for (int nY = 0; nY < 3; ++nY)
			{
				for (int nX = 0; nX < 5; ++nX)
				{
					...
					if (pItem && pItem->m_GCObj.dwColor == Red)
						pItem->m_GCObj.dwColor = White;
				}
			}
		}
		pMyGold->SetText((char*)"         0", 0);
...
void TMFieldScene::SetVisiblePotal(int bShow, int nPos)
{
	...
	if (bShow)
	{
		...
		m_pPotalText->SetTextColor(White);
	}
	else
	{
		memset(&m_stPotalItem, 0, sizeof(m_stPotalItem));
...
void TMFieldScene::SetInVisibleInputCoin()
{
	...
	if (m_nLastAutoTradePos >= 0)
	{
		m_pCargoGridList[m_nLastAutoTradePos / 40]->GetAtItem(m_nLastAutoTradePos % 40 % 5,	m_nLastAutoTradePos % 40 / 5)->m_GCObj.dwColor = White;
...
void TMFieldScene::UpdateScoreUI(unsigned int unFlag)
{
	...
	if (!(unFlag & 0x10))
	{
		...
		if (m_pCIFakeExp)
		{
			...
			if (nLevelCount < 80)
				m_pCIFakeExp->SetTextColor(White);
			else
				m_pCIFakeExp->SetTextColor(Red);
		}
		if (m_pExpHold)
		{
		...
		if (CurLevel >= 401)
			sprintf(szStr, "4,290,000,000", g_pNextLevel[CurLevel - 1]);
		...
		int Precision = BASE_GetMobAbility(pMobData, EF_HITRATE);
		...
		sprintf(szStr, "%d", BASE_GetMobAbility(pMobData, EF_SAVEMANA));
		if (m_pSaveMana)
			m_pSaveMana->SetText(szStr, 0);

		sprintf(szStr, "%d", BASE_GetMobAbility(pMobData, EF_HPADD));
		m_pRegenHP->SetText(szStr, 0);
		if (m_pRegenHP)
			m_pRegenHP->SetText(szStr, 0);

		sprintf(szStr, "%d", BASE_GetMobAbility(pMobData, EF_MPADD));
		...
		sprintf(szStr, "%d", g_pSpell[cSkillIndex].Range);
		if (m_pAttackRange)
			m_pAttackRange->SetText(szStr, 0);

		sprintf(szStr, "%d", g_pSpell[cSkillIndex].Delay); //new
		if (m_pSkillDelayDec)
		...
		auto labelcash = (SText*)m_pControlContainer->FindControl(3000069); //qual � a label la do cahs?
...
void TMFieldScene::OnESC()
{
	...
	else if (m_pSkillPanel->IsVisible() == 1)	 // SKILL
	{
		OnControlEvent(65568u, 0);
	}
	else if (m_pShopPanel->IsVisible() == 1 && m_pInvenPanel->IsVisible() == 1)
	{
		SetVisibleShop(0);
		OnControlEvent(65562u, 0);
	}
	...
	// PROTEÇÃO INVENTÁRIO FECHANDO CARGO
	else if (m_pCargoPanel->IsVisible() == 1 ||
		(m_pInvenPanel->IsVisible() == 1 && m_pCargoPanel->IsVisible() == 1))
	{
		CloseCargoSafe();
		OnControlEvent(65562u, 0);
	}
	...
	else if (m_pDropPanel[49]->IsVisible() == 1)//aqui sistema esc do droplist
	{
		m_pDropPanel[49]->SetVisible(0);
	}
	...
	if (m_pAutoTrade)
	{
		...
		else
		{
			...
			for (int k = 0; k < 10; ++k)
			{
				...
				if (pSrcItem)
					pSrcItem->m_GCObj.dwColor = White;
...
int TMFieldScene::LoadMsgText(SListBox* pListBox, char* szFileName)
{
	...
	for (int i = 0; i < 100 && fgets(szTemp, 256, fp); ++i)
	{
		...
		if (szTemp[6] == ' ')
		{
			...
			pListBox->AddItem(new SListBoxItem(szText, dwCol | Black, 0.0f, 0.0f, 400.0f, 16.0f, 0, 0x77777777, 1u, 0));
		}
	}
...
int TMFieldScene::OnMsgBoxEvent(unsigned int idwControlID, unsigned int idwEvent, unsigned int dwServerTime)
{
	...
	case 38:
	{
		int nCellX = (m_pMessageBox->m_dwArg & Red) >> 16;
		int nCellY = m_pMessageBox->m_dwArg & 0xFFFF;
...
int TMFieldScene::OnKeyPlus(char iCharCode, int lParam)
{
	...
	++m_nChatListSize;
	m_nChatListSize %= 4; // 0,1,2,3 (3 NÃO fecha)

	if (!m_pChatList || !m_pChatBack || !m_pControlContainer)
		return 0;

	SListBox* pChatList = m_pChatList;

	// Força visível (não deixa "sumir" no step 3)
	if (!m_pChatBack->m_bVisible)
		m_pChatBack->SetVisible(1);

	if (!pChatList->m_bVisible)
		pChatList->SetVisible(1);

	// ====== Guarda estado atual do scroll ======
	int oldVisible = pChatList->m_nVisibleCount;
	int oldStart = pChatList->m_nStartItemIndex;
	int numItems = pChatList->m_nNumItem;

	int oldMaxStart = numItems - oldVisible;
	if (oldMaxStart < 0) oldMaxStart = 0;
	bool wasAtBottom = (oldStart >= oldMaxStart);

	// ====== Altura alvo ======
	float newListH = (float)(140 * m_nChatListSize + 60) * RenderDevice::m_fHeightRatio;
	if (newListH < 10.0f)
		newListH = (float)(60.0f * RenderDevice::m_fHeightRatio);

	// ====== Ajuste CRÍTICO: visibleCount + startIndex sem buracos ======
	pChatList->m_nVisibleCount = 10 * m_nChatListSize + 5;

	numItems = pChatList->m_nNumItem;
	int maxStart = numItems - pChatList->m_nVisibleCount;
	...
	pChatList->SetStartItemIndex(newStart);

	if (pChatList->m_pScrollBar)
	{
		pChatList->m_pScrollBar->SetMaxValue(numItems);
		pChatList->m_pScrollBar->SetCurrentPos(pChatList->m_cEditable + newStart);
		pChatList->m_pScrollBar->Update();
	}

	// ====== ANIMAÇÃO SUAVE (não aplica SetSize agora — aplica no update por frame) ======
	float fromH = pChatList->m_nHeight;
	if (fromH < 10.0f)
		fromH = (float)(60.0f * RenderDevice::m_fHeightRatio);

	m_dwChatResizeStart = g_pTimerManager->GetServerTime();
	m_fChatResizeFromH = fromH;
	m_fChatResizeToH = newListH;
	m_bChatResizeAnim = true;
...
int TMFieldScene::OnKeyPlus(char iCharCode, int lParam)
{
	...
	// ============================================================
	// 1) Garante visibilidade (não deixa o step 3 "sumir")
	// ============================================================
	if (!m_pChatBack->m_bVisible)
		m_pChatBack->SetVisible(1);
	...
	// ============================================================
	// 2) Salva estado atual do scroll
	// ============================================================
	const int oldVisibleCount = pChatList->m_nVisibleCount;
	const int oldStartIndex = pChatList->m_nStartItemIndex;
	int       numItems = pChatList->m_nNumItem;

	int oldMaxStart = numItems - oldVisibleCount;
	if (oldMaxStart < 0) oldMaxStart = 0;

	const bool wasAtBottom = (oldStartIndex >= oldMaxStart);

	// ============================================================
	// 3) Calcula altura alvo do chat (baseado no step atual)
	// ============================================================
	float targetHeight = (float)(140 * m_nChatListSize + 60) * RenderDevice::m_fHeightRatio;

	// Segurança: evita altura inválida em estados bugados
	const float kMinHeight = (float)(60.0f * RenderDevice::m_fHeightRatio);
	if (targetHeight < 10.0f)
		targetHeight = kMinHeight;

	// ============================================================
	// 4) Ajuste CRÍTICO: visibleCount e startIndex sem "buracos"
	// ============================================================
	pChatList->m_nVisibleCount = 10 * m_nChatListSize + 5;
	...
	// Se estava no fim, continua no fim após redimensionar
	if (wasAtBottom)
		newStartIndex = maxStart;

	// Clamp
	if (newStartIndex > maxStart) newStartIndex = maxStart;
	if (newStartIndex < 0)        newStartIndex = 0;

	// Aplica no list
	pChatList->SetStartItemIndex(newStartIndex);

	// Atualiza scrollbar junto
	if (pChatList->m_pScrollBar)
	{
		pChatList->m_pScrollBar->SetMaxValue(numItems);

		// OBS: o SListBox soma m_cEditable quando existe linha de edição
		pChatList->m_pScrollBar->SetCurrentPos(pChatList->m_cEditable + newStartIndex);
		pChatList->m_pScrollBar->Update();
	}
	...
	m_dwChatResizeStart = g_pTimerManager->GetServerTime();
	m_fChatResizeFromH = fromHeight;
	m_fChatResizeToH = targetHeight;
	m_bChatResizeAnim = true;
...
int TMFieldScene::OnPacketMessageWhisper(MSG_MessageWhisper* pMsg)
{
	...
	else if (pMsg->String[0] == '!')
	{
		...
		m_pHelpList[3]->AddItem(new SListBoxItem((char*)" ", White, 0.0f, 0.0f, 300.0f, 16.0f, 0, 0x77777777, 1, 0));
		...
		m_pHelpList[3]->AddItem(new SListBoxItem(szTime, White, 0.0f, 0.0f, 300.0f, 16.0f, 0, 0x77777777, 1, 0));
...
int TMFieldScene::OnPacketCNFRemoveServer(MSG_CNFRemoveServer* pStd)
{
	if (pStd->Header.ID != g_pObjectManager->m_dwCharID || g_pSocketManager->Sock)
	{
		...
		if (g_pSocketManager->ConnectServer(g_pApp->m_szServerIP, TM_CONNECTION_PORT, 0, 1124))
		{
			...
			stAccountLogin.ClientVersion = 759;
...
int TMFieldScene::OnPacketCNFMobKill(MSG_CNFMobKill* pStd)
{
	...
	if (pAttacker)
	{
		...
		m_pEffectContainer->AddChild(new TMEffectCharge(pAttacker, 0, White));
	}
...
int TMFieldScene::OnPacketAttack(MSG_STANDARD* pStd)
{
	...
	if (pAttacker)
	{
			...
			pAttacker == m_pMyHuman && (unsigned char)pAttack->Motion == 254)
		{
			...
			else if (pAttack->SkillIndex == 3) // Persegui��o
			{
				if (pAttacker)
				{
					TMVector3 vecPos{ pAttacker->m_vecPosition.x, pAttacker->m_fHeight + 1.0f, pAttacker->m_vecPosition.y };
			...
			else if (pAttack->SkillIndex == 77) // Medita��o
			{
			...
			else if (pAttack->SkillIndex == 100) // Ressurei��o
			{
				GetSoundAndPlay(156, 0, 0);
			}
					...
					if (pAttack->Dam[i].Damage == -3 || pAttack->Dam[i].Damage == -4)
					{
						...
						if (BASE_Get3DTo2DPos(pTargetHuman->m_vecPosition.x, pTargetHuman->m_fHeight + 1.0f, pTargetHuman->m_vecPosition.y, &nTX, &nTY))
						{
							...
							if (bDrawFront)
							{
								...
								auto pFont = new TMFont3(szStr, nTX, nTY + (int)(RenderDevice::m_fHeightRatio * 80.0f), White, 2.0f, dwDelay, 1, 1500, 0, 4);
					...
					if (pAttack->Dam[i].Damage == -3 || pAttack->Dam[i].Damage == -4)
					{
						...
						if (BASE_Get3DTo2DPos(pTargetHuman->m_vecPosition.x, pTargetHuman->m_fHeight + 1.0f, pTargetHuman->m_vecPosition.y, &pX, &pY))
						{
							...
							if (bDrawFront)
							{
								char szStr[128]{};
								sprintf(szStr, "miss");
								auto pFont = new TMFont3(szStr, pX, pY + (int)(RenderDevice::m_fHeightRatio * 80.0f), White, 2.0f, dwDelay, 1, 1500, 0, 4);
								m_pExtraContainer->AddChild(pFont);
							}
						}
					}
...
void TMFieldScene::ClearInventorySelectedItem()
{
	for (int i = 0; i < 4; i++)
	{
		for (int slots = 0; slots < m_pGridInvList[i]->m_nNumItem; slots++)
			m_pGridInvList[i]->m_pItemList[slots]->m_GCObj.dwColor = White;
	}
}
	...
	if (page == 0)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			if (lb[i] == 478487)
				...
				sprintf(str, "Posi��o: %d %d", it->X, it->Y);
			else if (lb[i] == 478489)
...
int TMFieldScene::OnPacketInforPlay(stSendInfoClient* pStd)
{
...
{
	if (szStr)
		//sprintf(szStr, "%c01%02d%04d.bmp", 'w',  nChannel, nGuild);
		//sprintf(szStr, "%c %02d %02d %04d.bmp", 'w', g_pObjectManager->m_nServerGroupIndex, nChannel, nGuild);
		sprintf(szStr, "%c%02d%02d%04d.bmp", 'w', g_pObjectManager->m_nServerGroupIndex, 0, nGuild);
}
...
void TMFieldScene::Bag_View()
{
	...
	if (g_pObjectManager->m_stMobData.Carry[60].sIndex == 3467)
	{
		...
		m_pJPNBag_Day1->SetTextColor(Yellow);
	}
	else if (m_bJPNBag[2])
	{
		m_pInvPageBtn3->SetTextureSetIndex(548);
	...
	if (g_pObjectManager->m_stMobData.Carry[61].sIndex == 3467)
	{
		...
		m_pJPNBag_Day2->SetTextColor(Yellow);
	}
	else if (m_bJPNBag[3])
	{
		m_pInvPageBtn4->SetTextureSetIndex(548);
	...
	for (int i = 0; i < 32; ++i)
	{
		if ((unsigned char)m_pMyHuman->m_stAffect[i].Type <= 50)
		{
			...
			if (m_pAffectIcon[i] && i && (unsigned char)m_pMyHuman->m_stAffect[i].Type > 0 || !i && nTime > 0)
			{
				...
				m_pAffectIcon[i]->m_nPosY = 60.0f * RenderDevice::m_fWidthRatio;// corre��o das skils
...
void TMFieldScene::JumpMiniPanel()
{
	if (!m_pMiniPanel)
		return;

	float screenWidth = BASE_ScreenResize(800.0f);

	m_fMiniPanelTargetX = screenWidth - m_pMiniPanel->m_nWidth - BASE_ScreenResize(5.0f);
	m_fMiniPanelAnimX = screenWidth + BASE_ScreenResize(40.0f);

	m_fMiniPanelSpeedOpen = BASE_ScreenResize(35.0f);

	m_pMiniPanel->m_nPosX = m_fMiniPanelAnimX;
	m_pMiniPanel->SetVisible(1);

	m_bMiniPanelClosing = 0;
	m_bMiniPanelAnimating = 1;
}

void TMFieldScene::CloseMiniPanel()
{
	if (!m_pMiniPanel)
		return;
	...
	m_fMiniPanelTargetX = screenWidth + BASE_ScreenResize(40.0f);
	m_fMiniPanelAnimX = m_pMiniPanel->m_nPosX;

	m_fMiniPanelSpeedClose = BASE_ScreenResize(25.5f);

	m_bMiniPanelClosing = 1;
	m_bMiniPanelAnimating = 1;
}

void TMFieldScene::OpenInventoryAnimated()
{
	if (!m_pInvenPanel) return;

	float target = RenderDevice::m_fWidthRatio * 650.0f;

	m_pInvenPanel->SetVisible(1);

	m_fInvenTargetX = target;
	m_bInvenClosing = 0;
	m_bInvenAnimating = 1;
}

void TMFieldScene::CloseInventoryAnimated()
{
	if (!m_pInvenPanel) return;

	float screenRight = (float)g_pDevice->m_dwScreenWidth;

	m_fInvenTargetX = screenRight + 20.0f;
	m_bInvenClosing = 1;
	m_bInvenAnimating = 1;
}

void TMFieldScene::InitCPanelAnim()
{
	if (!m_pControlContainer)
		return;
	...
	// velocidades (ajuste fino)
	m_fCPanelSpeedOpen = BASE_ScreenResize(35.0f);
	m_fCPanelSpeedClose = BASE_ScreenResize(35.0f);

	// posição ABERTA = onde ele já está no layout original
	m_fCPanelOpenX = m_pCPanel->m_nPosX;

	// posição FECHADA = totalmente fora da tela à direita
	float screenW = (float)RenderDevice::m_dwCurrScreenX;
	m_fCPanelCloseX = screenW + BASE_ScreenResize(10.0f);

	// começa fechado? (opcional)
	// Se você quer que ele inicie fechado fora da tela:
	m_fCPanelAnimX = m_fCPanelCloseX;
	m_pCPanel->m_nPosX = m_fCPanelAnimX;
	m_pCPanel->SetVisible(0);
...
void TMFieldScene::ToggleCPanel()
{
	if (!m_pControlContainer)
		return;

	// pega sempre do container (seguro)
	SPanel* pPanel = (SPanel*)m_pControlContainer->FindControl(65696);
	if (!pPanel)
		return;
	...
	// recalcula X de fechar caso a resolução mude
	float screenW = (float)RenderDevice::m_dwCurrScreenX;
	m_fCPanelCloseX = screenW + BASE_ScreenResize(10.0f);

	// se estiver animando, ignora (ou você pode inverter)
	if (m_bCPanelAnimating)
		return;

	// Se está visível => vai fechar
	if (m_pCPanel->m_bVisible)
	{
		m_bCPanelClosing = 1;
		m_bCPanelAnimating = 1;

		// começa do X atual (pra não dar salto)
		m_fCPanelAnimX = m_pCPanel->m_nPosX;
	}
	else // está invisível => vai abrir
	{
		m_bCPanelClosing = 0;
		m_bCPanelAnimating = 1;

		// IMPORTANTE: deixa visível ANTES de animar
		m_pCPanel->SetVisible(1);

		// começa de fora da tela (fechado)
		m_fCPanelAnimX = m_fCPanelCloseX;
		m_pCPanel->m_nPosX = m_fCPanelAnimX;
	}
}

void TMFieldScene::ToggleCharInfoAnimated()
{
	if (!m_pControlContainer)
		return;
	...
	// evita spam no meio da animação
	if (m_bCPanelAnimating)
		return;

	// ===== posições alvo (mesmo padrão do seu OnKeyVisibleCharInfo) =====
	const float padLeft = BASE_ScreenResize(65.0f);     // X aberto
	const float offPad = BASE_ScreenResize(10.0f);
	const float closeX = -(float)m_pCPanel->m_nWidth - offPad; // fora da tela à esquerda
	...
	if (m_pCPanel->m_bVisible)
	{
		...
		m_fCPanelAnimX = (float)m_pCPanel->m_nPosX; // começa do X atual
		m_fCPanelTargetX = closeX;                    // destino fechado
	}
	else
	{
		// ABRIR
		...
		m_fCPanelAnimX = closeX;   // começa fora (esquerda)
		m_fCPanelTargetX = padLeft;  // destino aberto

		m_pCPanel->m_nPosX = (int)m_fCPanelAnimX;
	}

	// botão (B_CHAR = 65790 no teu projeto)
	SButton* pBtnChar = (SButton*)m_pControlContainer->FindControl(B_CHAR);
	if (pBtnChar)
		pBtnChar->SetSelected(m_bCPanelClosing ? 0 : 1);
