...
constexpr auto NUM_ITEM_DESC_PARAMS = 14;
...
public:
	...
	void CloseCargoSafe(); // Resolve bug do cargo abrindo paineis
	void ClearCombine();
	void ClearCombine4();
	void DoCombine();
	void DoCombine4();
	...
	int OnPacketInforPlay(stSendInfoClient* pStd);
	int OnPacketBattle(MSG_TowerWar* pStd);
	int OnPacketMacroWater(stWaterScrollMacro* pStd);
	int OnPacketSendExpMsg(MSG_Exp_MsgPanel* pStd);
	int OnPacketNewCashRev(PacketRevDonate* P);
	int OnPacketNewCashRev2(PacketRevDonate2* pStd);
	void  UpdateNewStore(int idwControlID);
	void BuyItemNewStore(int idwControlID);

	
	int OnPacketNewBuyCash(MSG_STANDARD* pStd);
	...
	void CloseMiniPanel();
	 ...
	 // EFEITO INVENTARIO
	void OpenInventoryAnimated();
	void CloseInventoryAnimated();

	void UpdateChatResizeAnim();
...
public:
	...
	// CPanel anim (id 65696)

	int     m_bCPanelAnimating = 0;
	int     m_bCPanelClosing = 0;

	float   m_fCPanelAnimX = 0.0f;
	float   m_fCPanelOpenX = 0.0f;   // X quando aberto (na tela)
	float   m_fCPanelCloseX = 0.0f;  // X quando fechado (fora da tela)

	float   m_fCPanelSpeedOpen = 0.0f;
	float   m_fCPanelSpeedClose = 0.0f;
	float m_fCPanelTargetX;

	// MINI PANEL ANIMATION
	SPanel* m_pMiniPanel;
	float m_fMiniPanelAnimX;				 
	float m_fMiniPanelTargetX;	
	int   m_bMiniPanelAnimating; 
	int m_bMiniPanelClosing;  
	float m_fMiniPanelSpeedOpen;
	float m_fMiniPanelSpeedClose;


	// INVENTORY ANIMATION
	float m_fInvenAnimX;
	float m_fInvenTargetX;
	float m_fInvenSpeedOpen;
	float m_fInvenSpeedClose;
	int   m_bInvenAnimating;
	int   m_bInvenClosing;
	...
	SGridControl* m_pCargoGridList[4];
	...
	SButton* m_pBTNCLOSEINVENT;
	SButton* m_pBTNgoldINVENT;
	SButton* m_btncloseskill;
	SButton* m_pSlotGuardaCarga[40];
	SButton* m_pSlotSkill[47];
	SButton* m_pSlotShop[40];
	SButton* m_pSlotInventori[32];
	SButton* m_pSlotSkillShop[24];
	SButton* m_pSlotComp[33];
	SButton* m_pSlotTrade[30];
	SGridControl* m_pGridResultItem[35];
	SPanel* m_pMixPanel2;
	SPanel* m_pMixPanel1;
	SPanel* m_pMixPanel[17];
	SPanel* m_pSelectServerPanel;
	SPanel* m_plojapessoal;
	SPanel* m_pDropListPanel;
	SPanel* m_pPotalPanel11;
	SListBox* m_ListBoxPanel;
	SGridControl* m_GridPanel;

	SPanel* m_GridPanel1;
	SButton* m_pPanelSend;
	SButton* m_pbutonShop;
	SButton* m_pbutonDrop;
	SButton* m_pPanelDropClose;
	SText* m_pMixPanelTextDrop[11];
	SButton* m_pbutonNewShop[13];

	SPanel* m_pItemMixPanel45;
	SButton* m_pCompPanel[3];
	SButton* m_pCompbuton;
	SButton* m_pCompbuton1;
	SText* m_pCompvalue;
	SText* m_pComptext;
	SText* m_pMixPanelText;
	SText* m_pMixPanelTextbuton;
	SPanel* m_pccmode;
	SText* m_pmsgskill;
	...
	SText* m_pRegenHP;
	SText* m_pRegenMP;
	SText* m_pCriticalDamInc;
	SText* m_pAttackRange;
	SText* m_pSkillDelayDec;
	SText* m_pCriticalNew;
	SText* m_pAtkMagic;
	SText* m_pBonusEXP;
	SText* m_pBonusDROP;
	SText* m_pNickDROP[7];
