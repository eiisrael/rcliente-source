...
TMHuman::TMHuman(TMScene* pParentScene)
{
    ...
    if (g_pCurrentScene != nullptr)
    {
        ...
        m_pAutoTradeDesc = new SText(-1, "", White, 0.0, 650.0f, 143.0f, 50.0f, 0, White, 1, 0);
        ...
        m_pChatMsg = new SText(-2, "", White, 0.0f, 650.0f, 256.0f, 64.0f, 1, 0x77000000, 1, 0);
       ...
       // m_pTitleProgressBar1 = new SProgressBar(172, 0, 0, -36.0f, 0.5f, 36.0f, 16.0f, 0xFF800000, 0x40333333u, 1);
        ...
        //m_pTitleProgressBar->AddChild(m_pTitleProgressBar1);
...
int TMHuman::InitObject()
{
    ...
    if (m_sCostume == 4384)//skin de arma
    {
        m_stLookInfo.LeftMesh = 2987;
    }

    int nCos = 0;
    if (m_sCostume >= 4150 && m_sCostume < 4200 || m_sCostume >= 4300 && m_sCostume <= 4500 && m_nClass != 29)
    {
...
int TMHuman::Render()
{
    ...
    if (!pMesh || nCommon != 2888 && nCommon != 2889)
    {
        ...
        else
        {
            ...
            else if (m_nMountSkinMeshType == 50)//balanço
            {
...
int TMHuman::OnPacketSetHpMp(MSG_SetHpMp* pStd)
{
    ...
    if (pFScene1->m_cAutoAttack == 1 && pFScene->m_pTargetHuman == this && pStd->Mp <= 0)
...
int TMHuman::OnPacketMessageChat(MSG_STANDARD* pStd)//mudar aqui pacote de chat recebido por outros players
{
    ...
    if (strlen(pMsgChat->String) + strlen(m_szName) <= 50)
    {
        ...
        auto ipNewItem = new SListBoxItem(szMsg, White, 0.0, 0.0, 280.0f, 16.0f, 0, 0x77777777u, 1u, 0);
    ...
    else
    {
        ...
        auto ipNewItem = new SListBoxItem(szMsg, White, 0.0, 0.0, 280.0f, 16.0f, 0, 0x77777777, 1u, 0);

        if (ipNewItem && pChatList)
            pChatList->AddItem(ipNewItem);

        auto ipNewItem2 = new SListBoxItem(szMsg2, White, 0.0, 0.0, 280.0f, 16.0f, 0, 0x77777777, 1u, 0);
...
int TMHuman::OnPacketMessageWhisper(MSG_MessageWhisper* pMsg)
{
    ...
    else if (pMsg->String[0] == '!')//alterado
    {
        ...
        if (pScene->m_pHelpList[3] != nullptr)
            pScene->m_pHelpList[3]->AddItem(new SListBoxItem(" ", White, 0.0f, 0.0f, 280.0f, 16.0f, 0, 0x77777777u, 1u, 0));
        ...
        if (pScene->m_pHelpList[3] != nullptr)
            pScene->m_pHelpList[3]->AddItem(new SListBoxItem(szTime, White, 0.0f, 0.0f, 280.0f, 16.0f, 0, 0x77777777u, 1u, 0));
...
int TMHuman::OnPacketUpdateEtc(MSG_STANDARD* pStd)
{
    ...
    if (g_pCurrentScene->m_pMyHuman == this)
    {
        ...
        if (g_pObjectManager->m_stMobData.LearnedSkill[0] != (long) pUpdateEtc->LearnedSkill[0])
        {
            if (g_pSoundManager)
            {
        ...
        g_pObjectManager->m_stMobData.LearnedSkill[0] = (int) pUpdateEtc->LearnedSkill[0];
...
void TMHuman::UpdateScore(int nGuildLevel)
{
    if (!m_dwDelayDel)
    {
        ...
        if (auto panelWarTower = (SPanel*)pScene->m_pControlContainer->FindControl(90624))
        {
            ...
            int X = pScene->m_pMyHuman->m_vecPosition.x;
            int Y = pScene->m_pMyHuman->m_vecPosition.y;
        ...
        if(pDest == nullptr)
        {
            ...
            {
                m_pNameLabel->m_cBorder = 1;
                m_pNameLabel->SetTextColor(Black);
            }
            else
            {
                ...
                m_pNameLabel->SetTextColor(dwB | dwG | dwR | Black);
        ...
        if (g_pCurrentScene->m_pMouseOverHuman == this && pFocused != this)
        {
            if ((m_dwID >= 0 && m_dwID < 1000) || (m_stScore.Reserved & 0xF) != 15)
            {
                ...
                else
                {
                    ...
                    if (bTown || m_pProgressBar->m_GCProgress.dwColor != Red)
            ...
            else
            {
                ...
                else
                {
                    ...
                    {                   
                        ...
                        m_pNameLabel->SetTextColor(((unsigned int)(float)(fProgress * 255.0f) << 16) | Black);
                    }
                    ...
                    if (sLevel - 40 < m_stScore.Level && 
                        m_sHeadIndex > 40 || m_stScore.Level >= 350 && m_sHeadIndex > 40)
                    {
                        m_pTitleProgressBar->m_GCProgress.dwColor = Red;
                    }

                 /*   if (m_sHeadIndex >= 40)
                    {
                       m_pTitleProgressBar1->SetVisible(1);
                    }*/
...
;
                        m_pProgressBar->m_GCProgress.dwColor = Red;
                        ...
                        {
                            m_pProgressBar->m_GCProgress.dwColor = Green;
                            m_pProgressBar1->m_GCProgress.dwColor = blueblue;
                            m_pTitleProgressBar->m_GCProgress.dwColor = Green;
                        }
...
void TMHuman::RenderEffect()//aqui
{
    ...
    else if (m_sCostume == 4376)
    {
        RenderEffect_RudolphCostume1(dwServerTime);
    }
...
void TMHuman::FrameMoveEffect(unsigned int dwServerTime)
{
    ...
    {      
            ...
            m_stEffectEvent.sEffectIndex == 104 || m_stEffectEvent.sEffectIndex == 105)
        {
            ...
            {
                auto pFire = new TMSkillFire(vecDest, 0, m_stEffectEvent.pTarget, White, 0x22331100);
                g_pCurrentScene->m_pEffectContainer->AddChild(pFire);
            }
            ...
            else if (m_stEffectEvent.sEffectIndex == 72 || m_stEffectEvent.sEffectIndex == 80)
            {
                if (m_stEffectEvent.pTarget)
                {
                    ...
                    const static unsigned int dwCol[2][2]{ White, 0xFFFFAA55, White, Red };
                    auto pCamera = g_pObjectManager->GetCamera();
                    if (pCamera->m_pFocusedObject == this)
    ...
    //if (m_cLighten == 1 && !g_bHideEffect) //effeito da skill trovão
    //{
   ...
   /* if (m_cShield == 1 && dwServerTime - m_dwLastMagicShield > 1000)//escudo magico
    ...
    if (m_sFamiliar == 753 && !m_pFamiliar)
    {
        ...
        if (m_pFamiliar)
        {//
            m_pFamiliar->m_stLookInfo.Mesh1 = 6;
            m_pFamiliar->m_stLookInfo.Mesh0 = 6;
...
void TMHuman::SetPacketEquipItem(unsigned short* sEquip)
{
    if (!m_dwDelayDel)
    {
        ...
        if ((int)sEquip[12] < 4150 || (int)sEquip[12] > 4200 && (int)sEquip[12] < 4300 || (int)sEquip[12] > 4500)
            m_sCostume = 0;
        else
...
void TMHuman::CreateControl()
{
    ...
    m_pAutoTradeDesc = new SText(-1, "", White, 0.0f, 650.0f, 143.0f, 50.0f, 0, White, 1u, 0);
    m_pAutoTradePanel = new SPanel(512, -10.0f, 635.0f, 141.0f, 43.0f, 0x77777777u, RENDERCTRLTYPE::RENDER_IMAGE_STRETCH);
    m_pAutoTradePanel->m_bSelectEnable = 0;

    m_pChatMsg = new SText(-2, "", White, 0.0, 650.0, (float)256, 64.0, 1, 0x77000000u, 1u, 0);
    m_pChatMsg->m_Font.m_bMultiLine = 1;
    ...
    m_pProgressBar = new SProgressBar(-2, 30, 30, 0.0f, 0.0f, 60.0f, 7.0f, Red, 0xFF333333, 1u);
    m_pProgressBar1 = new SProgressBar(-2, 30, 30, 0.0f, 0.0f, 60.0f, 7.0f, Red, 0xFF333333, 1u);
...
void  TMHuman::SetMountCostume(unsigned int  index)
{
    ...
    if (index >= 11 && index <= 200 && (!(m_stScore.Reserved & 2)))
    {
        ...
        case 54:
            m_nMountSkinMeshType = 31;
            m_stMountLook.Mesh0 = 50;
            m_stMountLook.Mesh1 = 50;
            m_fMountScale = 0.89f;
            m_stMountSanc.Sanc2 = 12;
            m_stMountSanc.Sanc1 = 12;
            m_stMountSanc.Sanc0 = 12;
            ...
            break;
        case 55://////////////////////////////
            m_nMountSkinMeshType = 25;
            m_stMountLook.Mesh0 = 5;
            m_stMountLook.Mesh1 = 5;
            m_fMountScale = 0.89f;
            m_stMountSanc.Sanc2 = 9;
            m_stMountSanc.Sanc1 = 9;
            m_stMountSanc.Sanc0 = 9;
            m_stOldMountSanc.Sanc2 = 9;
            m_stOldMountSanc.Sanc1 = 9;
            m_stOldMountSanc.Sanc0 = 9;
            break;
        case 56:
            m_fMountScale = 0.60000002f;
            m_stMountLook.Mesh0 = 11;
        ...
        case 63://4242
            m_nMountSkinMeshType = 31;
            m_stMountLook.Mesh0 = 51;
            m_stMountLook.Mesh1 = 51;
            m_fMountScale = 0.89f;
            m_stMountSanc.Sanc2 = 9;
            m_stMountSanc.Sanc1 = 9;
            m_stMountSanc.Sanc0 = 9;
        ...
        case 68://4246
            ...
            if (index == 68)
            {
                m_stMountLook.Skin0 = 2;
                m_stMountLook.Skin1 = 2;
            }
            break;
        case 69://4247
            break;
...
int TMHuman::SetHumanCostume()
{
    int nCos = 0;
    m_nSkinMeshType = 0;
    memset(&m_stColorInfo, 0, 6);//correção
    memset(&m_stColorInfo.Legend0, 0, 6);
        ...
        if (m_sCostume == 4150 || m_sCostume == 4151 || m_sCostume == 4169 || m_sCostume == 4170 ||
            m_sCostume == 4171 || m_sCostume == 4172 || m_sCostume == 4173 || m_sCostume == 4176 ||
            m_sCostume == 4177 || m_sCostume == 4178 || m_sCostume == 4179 || m_sCostume == 4183)
        {
            memset(&m_stSancInfo.Sanc0, 9, 6);
            //memset(&m_stSancInfo.Legend0, 0, 6u);
        }
        else if (m_sCostume == 4300 || m_sCostume == 4301 || m_sCostume == 4302 || m_sCostume == 4303 ||
            m_sCostume == 4304 || m_sCostume == 4305 || m_sCostume == 4309 || m_sCostume == 4310 ||
            m_sCostume == 4311 || m_sCostume == 4312 || m_sCostume == 4313 || m_sCostume == 4314||
            m_sCostume == 4315 || m_sCostume == 4316 || m_sCostume == 4317 || m_sCostume == 4318||
            m_sCostume == 4320 || m_sCostume == 4321 || m_sCostume == 4322 || m_sCostume == 4323||
            m_sCostume == 4324 || m_sCostume == 4325 || m_sCostume == 4326 || m_sCostume == 4327||
            m_sCostume == 4328 || m_sCostume == 4329 || m_sCostume == 4330 || m_sCostume == 4331||
            m_sCostume == 4332 || m_sCostume == 4333 || m_sCostume == 4334 || m_sCostume == 4335||
            m_sCostume == 4336 || m_sCostume == 4337 || m_sCostume == 4338 || m_sCostume == 4339||
            m_sCostume == 4340 || m_sCostume == 4341 || m_sCostume == 4342 || m_sCostume == 4343||
            m_sCostume == 4344 || m_sCostume == 4345 || m_sCostume == 4346 || m_sCostume == 4347||
            m_sCostume == 4348 || m_sCostume == 4349 || m_sCostume == 4350 || m_sCostume == 4351 ||
            m_sCostume == 4352 || m_sCostume == 4353 || m_sCostume == 4354 || m_sCostume == 4355 ||
            m_sCostume == 4357 || m_sCostume == 4358 || m_sCostume == 4359 || m_sCostume == 4360 ||
            m_sCostume == 4361 || m_sCostume == 4362 || m_sCostume == 4363 || m_sCostume == 4364 ||
            m_sCostume == 4365 || m_sCostume == 4366 || m_sCostume == 4367 || m_sCostume == 4368 ||
            m_sCostume == 4369 || m_sCostume == 4370 || m_sCostume == 4371 || m_sCostume == 4372 ||
            m_sCostume == 4356 || m_sCostume == 4373 || m_sCostume == 4374 || m_sCostume == 4376 ||
            m_sCostume == 4377 || m_sCostume == 4378 || m_sCostume == 4379 || m_sCostume == 4380 ||
            m_sCostume == 4381 || m_sCostume == 4382 || m_sCostume == 4383 || m_sCostume == 4384 ||
            m_sCostume == 4385 || m_sCostume == 4386 || m_sCostume == 4387 || m_sCostume == 4388 || 
            m_sCostume == 4389 || m_sCostume == 4390 || m_sCostume == 4391 || m_sCostume == 4392 || 
            m_sCostume == 4393 || m_sCostume == 4394 || m_sCostume == 4395 || m_sCostume == 4396 ||
            m_sCostume == 4397 || m_sCostume == 4398 || m_sCostume == 4399 || m_sCostume == 4400 ||
            m_sCostume == 4401 || m_sCostume == 4402 || m_sCostume == 4403 || m_sCostume == 4404 ||
            m_sCostume == 4405 || m_sCostume == 4406 || m_sCostume == 4407 || m_sCostume == 4408 || 
            m_sCostume == 4409 || m_sCostume == 4410 || m_sCostume == 4411 || m_sCostume == 4412 ||
            m_sCostume == 4413 || m_sCostume == 4414 || m_sCostume == 4415 || m_sCostume == 4416 ||
            m_sCostume == 4417 || m_sCostume == 4418 || m_sCostume == 4419 || m_sCostume == 4420
            || m_sCostume == 4421 || m_sCostume == 4422|| m_sCostume == 4423|| m_sCostume == 4424
            || m_sCostume == 4425|| m_sCostume == 4426 || m_sCostume == 4427 || m_sCostume == 4428
            || m_sCostume == 4429 || m_sCostume == 4430 || m_sCostume == 4431 || m_sCostume == 4432
            || m_sCostume == 4433)
        {
            memset(&m_stSancInfo.Sanc0, 9, 6);
           // memset(&m_stSancInfo.Legend0, 0, 6u);
...
void TMHuman::RenderEffect_RudolphCostume1(unsigned int dwServerTime)
{
    if ((dwServerTime - m_dwGolemDustTime) > 980)
    {
        auto pSpChange = new TMSkillSpChange1({ m_vecPosition.x, m_fHeight, m_vecPosition.y }, 0, this);
       
        auto pSpChange1 = new TMSkillSpChange1({ m_vecPosition.x, m_fHeight, m_vecPosition.y },1, this);
        g_pCurrentScene->m_pEffectContainer->AddChild(pSpChange1);
...
void TMHuman::RenderEffect_RudolphCostume(unsigned int dwServerTime)
{
    if ((dwServerTime - m_dwGolemDustTime) > 800)
    {
        ...
        if (pEffect != nullptr)
        {
            ...
            pEffect->m_efAlphaType = EEFFECT_ALPHATYPE::EF_BRIGHT;
            pEffect->m_vecPosition = TMVector3{ m_vecTempPos[1000].x, m_vecTempPos[10000].y, m_vecTempPos[10000].z };
            g_pCurrentScene->m_pEffectContainer->AddChild(pEffect);
        }
