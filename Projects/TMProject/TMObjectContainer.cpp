...
int TMObjectContainer::Load(const char* szFileName)
{
	...
	for (m_nObjectIndex = 0; m_nObjectIndex < MAX_OBJECT_LIST && pos < sz; ++m_nObjectIndex)
	{
		...
		else if (dwObjType >= 311 && dwObjType <= 322)
		{
		...
		else if (dwObjType >= 331 && dwObjType <= 342 || dwObjType >= 351 && dwObjType <= 378)
		...
		else if (dwObjType == 2035)
		{
			...
			if ((float)(m_fOffsetX + vecPosition.x) == 2366.5f && (float)(m_fOffsetY + vecPosition.y) == 3926.5f && g_pCurrentScene != nullptr)
			{
		...
		{
			TMEffectMesh* pChild = new TMEffectMesh(dwObjType, White, fAngle - 1.5707964f, 0);
			if (pChild)
			{
				pChild->m_fScaleH = 1.0f;
				pChild->m_fScaleV = 1.0f;
		...
		if (dwObjType == 2035 && (float)(m_fOffsetX + vecPosition.x) == 2366.5f && (float)(m_fOffsetY + vecPosition.y) == 3926.5f && g_pCurrentScene)
		{
			TMFieldScene* pScene = (TMFieldScene*)g_pCurrentScene;
