...
TMScene::TMScene() : TreeNode(0)
{
	...
	m_pTextBillMsg = new SText(-2, "ºô", White, 120.0f, 70.0f, 540.0f, 20.0f, 1, 0xAAFF0000, 1, 1);
	...
	m_pAlphaNative = new SText(-2, "BR", 0xFFFFFFDD, 0, fHeight, 28.0f, 16.0f, 1, 0xAAADFF2F, 1, 0);
	...
	if (!pStd->ID && (pStd->Type == MSG_MessagePanel_Opcode || pStd->Type == 0x102 || pStd->Type == 0x104 || pStd->Type == 0x105 || pStd->Type == 0x106))
	{
		...
		else if (pMsgPanel->String[0] == '!')
		{
			...
			if (g_pCurrentScene->m_eSceneType == ESCENE_TYPE::ESCENE_FIELD)
			{
				...
				auto pItem3 = new SListBoxItem(" ", White, 0.0f, 0.0f, 300.0f, 16.0f, 0, 0x77777777u, 1u, 0);
...
int TMScene::OnMouseEvent(unsigned int dwFlags, unsigned int wParam, int nX, int nY)
{
	...
	if (m_pControlContainer)
		uiBlock = m_pControlContainer->OnMouseEvent(dwFlags, wParam, nX, nY);
	  /*
	if (dwFlags == WM_LBUTTONDOWN)
	{
		if (uiBlock == 1)
			printf("CLICK BLOQUEADO PELA UI em (%d,%d)\n", nX, nY);
		else
			printf("CLICK NO MAPA (ANDA) em (%d,%d)\n", nX, nY);
	}	 */
