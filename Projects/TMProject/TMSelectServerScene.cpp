...
int TMSelectServerScene::InitializeScene()
{
	...
	m_pLoginPanelText->m_nPosY += 6.0f;
	...
	float nHeight = static_cast<float>(g_pDevice->m_dwScreenHeight) * 0.55f - static_cast<float>(m_pLoginPanel->m_nHeight) * 0.5f;
	m_pLoginPanel->SetPos(static_cast<float>(g_pDevice->m_dwScreenWidth) * 0.49f - m_pLoginPanel->m_nWidth * 0.5f, nHeight);

	if (g_pDevice->m_dwScreenWidth <= 1024)//alterado logo painel
	{
		float fWidthRatio = RenderDevice::m_fWidthRatio;
		float fHeightRatio = RenderDevice::m_fHeightRatio;
		pText->SetPos(pText->m_nPosX, static_cast<float>(g_pDevice->m_dwScreenHeight) - 400.0f);
		pText->SetPos(pText->m_nPosX, (float)(g_pDevice->m_dwScreenHeight) - 30.0f);

		m_pLogoPanels[0]->SetRealPos(204.0f * fWidthRatio, 10.0f * fHeightRatio);
		m_pLogoPanels[1]->SetRealPos(460 * fWidthRatio, 10.0f * fHeightRatio);
		...
		m_pLoginPanel->SetRealSize(370 * fWidthRatio, 275.0f * fHeightRatio);

		m_pLoginBtns[0]->m_nPosX = BASE_ScreenResize(50.1f);
		m_pLoginBtns[1]->m_nPosX = BASE_ScreenResize(100.1f);
		m_pLoginBtns[2]->m_nPosX = BASE_ScreenResize(150.1f);
		m_pLoginBtns[0]->m_nPosY = BASE_ScreenResize(118.1f);
		m_pLoginBtns[1]->m_nPosY = BASE_ScreenResize(150.1f);
		m_pLoginBtns[2]->m_nPosY = BASE_ScreenResize(118.1f);
		...
		m_pEditID->m_nPosY = BASE_ScreenResize(60.1f);
		m_pEditPW->m_nPosY = BASE_ScreenResize(85.1f);
		m_pEditID->m_nPosX = BASE_ScreenResize(110.1f);
		m_pEditPW->m_nPosX = BASE_ScreenResize(110.1f);
		m_pLoginPanelText->m_nPosX = BASE_ScreenResize(100.1f);
		m_pLoginIDText->m_nPosY = BASE_ScreenResize(60.1f);
		m_pLoginPasswordText->m_nPosY = BASE_ScreenResize(85.1f);
	}
	if (g_pDevice->m_dwScreenWidth == 1280)//alterado logo painel
	{
		float fWidthRatio = RenderDevice::m_fWidthRatio;
		float fHeightRatio = RenderDevice::m_fHeightRatio;
		pText->SetPos(pText->m_nPosX, static_cast<float>(g_pDevice->m_dwScreenHeight) - 400.0f);
		...
		m_pLoginPanel->SetRealSize(270 * fWidthRatio, 175.0f * fHeightRatio);
	}
	else
	{
		int nAddHeight = 0;
	...
	if (g_pDevice->m_dwScreenWidth <= 1024)//alterado painel login
	{
		float fWidthRatio = RenderDevice::m_fWidthRatio;
		float fHeightRatio = RenderDevice::m_fHeightRatio;
		m_pLoginPanel->SetRealSize(315 * fWidthRatio, 193.0f * fHeightRatio);
	}
	if (g_pDevice->m_dwScreenWidth == 600)
	{
		pText->SetPos(pText->m_nPosX, static_cast<float>(g_pDevice->m_dwScreenHeight) - 350.0f);
		pText->SetPos(pText->m_nPosX, (float)(g_pDevice->m_dwScreenHeight) - 30.0f);

	}
	if (g_pDevice->m_dwScreenWidth == 1024 && g_pDevice->m_dwScreenHeight == 768)
	{
		pText->SetPos(pText->m_nPosX, static_cast<float>(g_pDevice->m_dwScreenHeight) - 430.0f);
		pText->SetPos(pText->m_nPosX, (float)(g_pDevice->m_dwScreenHeight) - 30.0f);

	}
	if (g_pDevice->m_dwScreenWidth == 1280 && g_pDevice->m_dwScreenHeight == 600)
	{
		pText->SetPos(pText->m_nPosX, static_cast<float>(g_pDevice->m_dwScreenHeight) - 280.0f);
		pText->SetPos(pText->m_nPosX, (float)(g_pDevice->m_dwScreenHeight) - 30.0f);

	}
	if (g_pDevice->m_dwScreenWidth == 1280 && g_pDevice->m_dwScreenHeight == 600)//
	{
		pText->SetPos(pText->m_nPosX, static_cast<float>(g_pDevice->m_dwScreenHeight) - 360.0f);
		pText->SetPos(pText->m_nPosX, (float)(g_pDevice->m_dwScreenHeight) - 30.0f);

	}
	if (g_pDevice->m_dwScreenWidth == 1280 && g_pDevice->m_dwScreenHeight == 600)
	{
		pText->SetPos(pText->m_nPosX, static_cast<float>(g_pDevice->m_dwScreenHeight) - 510.0f);
		pText->SetPos(pText->m_nPosX, (float)(g_pDevice->m_dwScreenHeight) - 30.0f);

	}
	if (g_pDevice->m_dwScreenWidth == 1280 && g_pDevice->m_dwScreenHeight == 1024)
	{
		pText->SetPos(pText->m_nPosX, static_cast<float>(g_pDevice->m_dwScreenHeight) - 590.0f);
		pText->SetPos(pText->m_nPosX, (float)(g_pDevice->m_dwScreenHeight) - 30.0f);
	...
	if (pText)
	{
		float screenW = (float)g_pDevice->m_dwScreenWidth;
		float screenH = (float)g_pDevice->m_dwScreenHeight;

		// Y: colado no fim (ajuste fino com BASE_ScreenResize se quiser)
		float y = screenH - BASE_ScreenResize(20.0f);

		// X: centralizado
		float textW = pText->m_nWidth;
		if (textW < 1.0f)
			textW = 1.0f; // evita divisão/centro estranho se width vier 0

		float x = pText->m_nPosX + BASE_ScreenResize(55.0f); // direita +10px
		...
		if (pServerList)
		{
			...
			for (int num = 1;; ++num)
			{
				...
				if (g_pServerList[nIndexN][num][0])
				{
					...
					// -1??
					pServerItem[num] = new SListBoxServerItem(7, szStr, White, 0.0f, 0.0f, static_cast<float>(g_nChannelWidth), 16.0f, 499, 1, 0, nServerAge);
				...
				else if (m_bAdmit == 1 && num < m_nMaxGroup)
				{
					...
					pServerItem[num - 1] = new SListBoxServerItem(6, szStr, White, 0.0f, 0.0f, static_cast<float>(g_nChannelWidth), 16.0f, nUserCount2[num], 0, 0, 0);
...
void TMSelectServerScene::InitializeUI()
{
	...
	m_pNServerSelect->SetPos(((float)g_pDevice->m_dwScreenWidth - m_pNServerSelect->m_nWidth) * 0.5f,
		((float)g_pDevice->m_dwScreenHeight - m_pNServerSelect->m_nHeight) * 0.55f);
	...
	if (m_pNServerGroupList)
	{
		...
		for (int i = m_nMaxGroup; i >= 0; --i)
		{
			...
			if (count > -1 && g_pServerList[count][0][0])
			{
				...
				pGroupItem[i] = new SListBoxItem(szStr, White, 0.0f, 0.0f, 63.0f, 16.0f, 10.0f, 0x77777777u, 1u, 0);
				m_pNServerGroupList->AddItem(pGroupItem[i]);
			}
		}
	}
	m_pSelectSvPanel[0] = (SButton*)m_pControlContainer->FindControl(65544);
	m_pSelectSvPanel[1] = (SButton*)m_pControlContainer->FindControl(65545);
	m_pSelectSvPanel[2] = (SButton*)m_pControlContainer->FindControl(65546);

	m_pClseChanel = (SButton*)m_pControlContainer->FindControl(65539);
	m_pSelectChanel = (SText*)m_pControlContainer->FindControl(65541);
	m_pEnterChanel = (SText*)m_pControlContainer->FindControl(65540);
	m_pEnterChanel1 = (SButton*)m_pControlContainer->FindControl(B_SERVER_SEL_OK);

	if (g_pApp->m_dwScreenWidth <= 1024)
	{

		for (int i = 0; i < 3; ++i)
			m_pSelectSvPanel[i]->m_nWidth = BASE_ScreenResize(150.0f);
		for (int i = 0; i < 3; ++i)
			m_pSelectSvPanel[i]->m_nHeight = BASE_ScreenResize(18.0f);

		m_pEnterChanel1->m_nPosX = BASE_ScreenResize(55.1f);
		m_pEnterChanel1->m_nPosY = BASE_ScreenResize(232.1f);
		m_pEnterChanel->m_nPosX = BASE_ScreenResize(50.1f);
		m_pEnterChanel->m_nPosY = BASE_ScreenResize(10.1f);
		m_pClseChanel->m_nPosX = BASE_ScreenResize(220.1f);
		m_pClseChanel->m_nPosY = BASE_ScreenResize(232.1f);
		m_pSelectChanel->m_nPosX = BASE_ScreenResize(208.1f);
		m_pSelectChanel->m_nPosY = BASE_ScreenResize(10.1f);
		m_pSelectSvPanel[0]->m_nPosX = BASE_ScreenResize(11.5f);
		m_pSelectSvPanel[1]->m_nPosX = BASE_ScreenResize(11.5f);
		m_pSelectSvPanel[2]->m_nPosX = BASE_ScreenResize(149.1f);
		m_pSelectSvPanel[2]->m_nPosY = BASE_ScreenResize(5539.1f);
		m_pNServerSelect->m_nWidth = BASE_ScreenResize(330.0f);
		m_pNServerSelect->m_nHeight = BASE_ScreenResize(270.0f);
		m_pNServerSelect->m_nPosX = BASE_ScreenResize(250.0f);
		m_pNServerGroupList->m_nPosX = BASE_ScreenResize(12.0f);
		m_pNServerGroupList->m_nWidth = BASE_ScreenResize(150.0f);
		m_pNServerGroupList->m_nPosY = BASE_ScreenResize(41.0f);
		m_pNServerList->m_nPosX = BASE_ScreenResize(175.0f);
		m_pNServerList->m_nWidth = BASE_ScreenResize(150.0f);
		m_pNServerList->m_nPosY = BASE_ScreenResize(41.0f);
	...
	if (g_pApp->m_dwScreenWidth >= 1280)
	{
		...
		for (int i = 0; i < 3; ++i)
			m_pSelectSvPanel[i]->m_nWidth = BASE_ScreenResize(123.0f);

		m_pClseChanel->m_nPosX = BASE_ScreenResize(180.1f);
		m_pSelectChanel->m_nPosX = BASE_ScreenResize(169.1f);
		m_pSelectSvPanel[0]->m_nPosX = BASE_ScreenResize(11.5f);
		m_pSelectSvPanel[1]->m_nPosX = BASE_ScreenResize(11.5f);
		m_pSelectSvPanel[2]->m_nPosX = BASE_ScreenResize(149.1f);
		m_pSelectSvPanel[2]->m_nPosY = BASE_ScreenResize(5529.1f);
		m_pNServerSelect->m_nWidth = BASE_ScreenResize(280.0f);
		m_pNServerSelect->m_nPosX = BASE_ScreenResize(265.0f);
		m_pNServerGroupList->m_nPosX = BASE_ScreenResize(12.0f);
		m_pNServerGroupList->m_nWidth = BASE_ScreenResize(123.0f);
		m_pNServerList->m_nPosX = BASE_ScreenResize(150.0f);
		m_pNServerList->m_nWidth = BASE_ScreenResize(123.0f);
	}
}
...
void TMSelectServerScene::GameGradeScene()//new gamegrade
{
	...
	if (g_pApp->m_dwScreenWidth <= 1024)
	{
		...
		{
		case 1024u:
			m_GameGrade->m_nWidth = BASE_ScreenResize(800.0f);//alterado
			m_GameGrade->m_nHeight = BASE_ScreenResize(600.0f);//alterado
			break;
		case 640u:
			m_GameGrade->m_nWidth = BASE_ScreenResize(800.0f);//alterado
			m_GameGrade->m_nHeight = BASE_ScreenResize(600.0f);//alterado
	...
	if (g_pApp->m_dwScreenWidth == 1280 && g_pApp->m_dwScreenHeight == 600)
	{
		m_GameGrade->m_nWidth = BASE_ScreenResize(600.0f);//alterado
		m_GameGrade->m_nHeight = BASE_ScreenResize(450.0f);//alterado
	}

	if (g_pApp->m_dwScreenWidth == 1280 && g_pApp->m_dwScreenHeight == 600)
	{
		m_GameGrade->m_nWidth = BASE_ScreenResize(600.0f);//alterado
		m_GameGrade->m_nHeight = BASE_ScreenResize(500.0f);//alterado
	}

	if (g_pApp->m_dwScreenWidth == 1280 && g_pApp->m_dwScreenHeight == 960)
	{
		m_GameGrade->m_nWidth = BASE_ScreenResize(800.0f);//alterado
		m_GameGrade->m_nHeight = BASE_ScreenResize(600.0f);//alterado
	}
	if (g_pApp->m_dwScreenWidth == 1280 && g_pApp->m_dwScreenHeight == 1024)
	{
		m_GameGrade->m_nWidth = BASE_ScreenResize(800.0f);//alterado
		m_GameGrade->m_nHeight = BASE_ScreenResize(700.0f);//alterado
	}
	else if (g_pApp->m_dwScreenWidth == 1600)
	{
		m_GameGrade->m_nWidth = BASE_ScreenResize(800.0f);//alterado
		m_GameGrade->m_nHeight = BASE_ScreenResize(400.0f);//alterado
	}

	m_GameGrade->SetVisible(1);
	m_dGameGradePrintTime = 3000;
	m_bGameGradePlay = 1;
