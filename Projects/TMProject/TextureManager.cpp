...
{
	FILE* fpTxt = fopen("UI\\TextureListN.txt", "r");
	if (!fpTxt)
	{
		LOG_WRITELOG("Cannot open TextureListN.txt\n");
		return 0;
	}

	char str[512];
	while (true) {
		char* ret = fgets(str, 511, fpTxt);
		if (ret == nullptr)
			break;
		...
		int rtn = sscanf_s(str, "%d, %[^,], %c", &index, filename, sizeof(filename), &alphaFlag, sizeof(alphaFlag));

		if (rtn < 1) {
			MessageBoxA(NULL, " Can't parse strings on TextureListN.txt", NULL, MB_OK);
			continue;
		}
		if (index >= 512)
		{
			LOG_WRITELOG("Too many textures in TextureList.txt");
			break;
		}

		strncpy(m_stUITextureList[index].szFileName, filename, sizeof(m_stUITextureList[index].szFileName) - 1);
		m_stUITextureList[index].szFileName[sizeof(m_stUITextureList[index].szFileName) - 1] = '\0';

		m_stUITextureList[index].cAlpha = alphaFlag;
